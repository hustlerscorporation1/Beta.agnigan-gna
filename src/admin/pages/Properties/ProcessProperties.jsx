...
import { 
  RefreshCw, Search, Eye, MapPin, 
  Download, Calendar, DollarSign, Activity, AlertTriangle
} from 'lucide-react';
...
const ProcessProperties = () => {
  ...
  const fetchProcessProperties = async () => {
    try {
      setLoading(true);
  ...
  const exportData = () => {
    ...
    a.href = url;
    a.download = `terrains-en-processus-${Date.now()}.csv`;
    a.click();
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">{filteredProperties.length} terrain(s) en processus</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={fetchProcessProperties}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg">
          ...
          <p className="text-sm opacity-90">Terrains en Processus</p>
        </div>
        <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-xl p-6 text-white shadow-lg">
          ...
          <p className="text-sm opacity-90">Valeur en Processus</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Rechercher un terrain en processus..."
            value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredProperties.length > 0 ? (
            ...
            <div key={property.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
              <div className="relative h-48 bg-gray-200">
                ...
                <div className="absolute top-3 left-3">
                  <span className="px-3 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800 flex items-center gap-1 animate-pulse">
                    <Activity size={12} />
                    En Processus
                  </span>
                </div>
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-500 mb-4">
                  <Calendar size={14} />
                  <span>Démarré le {new Date(property.created_at).toLocaleDateString('fr-FR')}</span>
                </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-2 mb-3">
                  <p className="text-xs text-blue-800 flex items-center gap-1">
                    <Activity size={12} />
                    Processus administratif en cours
                  </p>
                </div>
                <button
          ...
          <div className="col-span-full bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p className="text-gray-500">Aucun terrain en processus d'achat trouvé</p>
          </div>
        )}
      </div>
    </div>
