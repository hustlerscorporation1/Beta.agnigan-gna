...
import { 
  ...
  Download, RefreshCw, Calendar, DollarSign, User
} from 'lucide-react';
...
const PurchasedProperties = () => {
  ...
  const exportData = () => {
    const csv = [
      ['Titre', 'Localisation', 'Prix', 'Surface', 'Date d\'achat'],
    ...
    a.href = url;
    a.download = `terrains-achetes-${Date.now()}.csv`;
    a.click();
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">{filteredProperties.length} terrain(s) acheté(s)</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={fetchPurchasedProperties}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg">
          ...
          <p className="text-sm opacity-90">Total Terrains Achetés</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredProperties.length > 0 ? (
            ...
            <div key={property.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-500 mb-4">
                  <Calendar size={14} />
                  <span>Acheté le {new Date(property.created_at).toLocaleDateString('fr-FR')}</span>
                </div>
                <button
          ...
          <div className="col-span-full bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p className="text-gray-500">Aucun terrain acheté trouvé</p>
          </div>
        )}
      </div>
    </div>
