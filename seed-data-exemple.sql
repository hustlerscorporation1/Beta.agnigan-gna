...
-- ================================================
-- 1. PROPRIÉTÉS D'EXEMPLE
-- ================================================

-- Supprimer les anciennes données de test (optionnel)
-- DELETE FROM properties WHERE title LIKE '%Exemple%' OR title LIKE '%Test%';
...
-- ================================================
-- 2. MESSAGES DE CONTACT D'EXEMPLE
-- ================================================

-- Insérer 20 messages d'exemple
...
(gen_random_uuid(), 'Amina Diallo', 'amina.d@yahoo.fr', '+228 92 34 56 78', 'Bonjour, le prix du terrain agricole à Kpalimé est-il négociable ? Je suis investisseur agricole et j''ai un projet sérieux.', 'unread', 'high', (SELECT id FROM properties WHERE title LIKE '%Terrain Agricole Kpalimé%' LIMIT 1), NOW() - INTERVAL '1 hour'),

-- Messages Non Lus (Priorité Moyenne)
...
(gen_random_uuid(), 'Pierre Koffi', 'pierre.k@gmail.com', '+228 95 67 89 01', 'Pourriez-vous m''indiquer si le duplex à Adidogomé est proche des écoles et centres commerciaux ? Merci.', 'unread', 'medium', (SELECT id FROM properties WHERE title LIKE '%Duplex Moderne Adidogomé%' LIMIT 1), NOW() - INTERVAL '4 hours'),

-- Messages Lus (Priorité Basse)
...
(gen_random_uuid(), 'Ibrahim Cissé', 'ibrahim.c@email.com', '+228 94 45 67 89', 'Je suis intéressé par plusieurs propriétés. Pouvez-vous me rappeler pour discuter ?', 'unread', 'high', NULL, NOW() - INTERVAL '30 minutes'),
...
(gen_random_uuid(), 'Thomas Gnassingbé', 'thomas.g@outlook.com', '+228 96 67 89 01', 'Proposition de partenariat commercial. Pouvons-nous organiser une rencontre ?', 'unread', 'high', NULL, NOW() - INTERVAL '1 hour'),

(gen_random_uuid(), 'Diane Lawson', 'diane.lawson@email.com', '+228 97 78 90 12', 'Question sur les documents nécessaires pour l''achat d''un terrain.', 'unread', 'low', NULL, NOW() - INTERVAL '8 hours'),

-- Messages Lus récents
...
-- Compter les propriétés insérées
SELECT 
    status,
    COUNT(*) as nombre,
    SUM(CAST(REPLACE(REPLACE(price, ' FCFA', ''), ' ', '') AS BIGINT)) as valeur_totale
FROM properties
WHERE title NOT LIKE '%Exemple%' OR title LIKE '%Villa%' OR title LIKE '%Terrain%'
GROUP BY status;

-- Compter les messages insérés
SELECT 
    status,
    COUNT(*) as nombre
FROM contacts
...
-- RÉSULTAT ATTENDU:
-- ================================================
-- ✅ 15 propriétés (7 disponibles, 3 vendues, 5 en attente)
-- ✅ 20 messages (7 non lus, 4 lus, 3 répondus, 3 archivés, 3 autres)
-- ✅ Données variées et réalistes
-- ✅ Relations property_id correctes
-- ✅ Dates échelonnées sur 30 jours
-- ✅ Images Unsplash fonctionnelles
-- ================================================

SELECT 'Données d''exemple créées avec succès! ✅' AS resultat;
